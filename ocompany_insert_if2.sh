#!/bin/bash
# syntax ./ocompany_insert_if2 CONFIG_1 CONFIG_2
#insert CONFIG_1 if CONFIG_2 is already present in .h file
#i.e.
#./ocompany_insert_if2 CONFIG_SPI_FLASH_GIGADEVICE CONFIG_SPI_FLASH_SST
#./ocompany_insert_if2 "CONFIG_USB_HOST_ETHER CONFIG_USB_ETHER_ASIX CONFIG_USB_ETHER_MCS7830 CONFIG_USB_ETHER_SMSC95XX" CONFIG_USB_MAX_CONTROLLER_COUNT
insert_configs=$1;
check_config=$2;
boards=`ls -d board/ocompany/* | sed 's.board/ocompany/..'`;
...
for board in ${boards} ; do
	...
	defconfigs="";
	hfile="";
	if [ -e board/ocompany/${board}/Kconfig ] ; then
		targets=`grep '^if TARGET_' board/ocompany/${board}/Kconfig | sed 's.if ..' | sed 's. ||. .'`;
		echo board=${board} target=${targets};
		hfile=`grep -A1 SYS_CONFIG_NAME board/ocompany/${board}/Kconfig | grep default | sed 's.default "..' | sed 's."..' | sed 's/[ \x09]*//'`;
